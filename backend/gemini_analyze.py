...
def _enforce_sofa_focal_orientation(result: dict, has_layout_ctx: bool) -> dict:
    """Enforce explicit sofa/window and sofa/focal orientation flags."""
...
"""
    ...
    result = _enforce_sofa_focal_orientation(result, has_layout_ctx)
    ...
    if has_layout_ctx:
        ...
        if is_window_side:
            ...
            # sofa threshold: normal window-side uses 60/65; dining-middle note tightens it.
            if isinstance(sofa_pct, (int, float)):
                if sofa_pct < sofa_hard_floor:
                    forced_solz = True
                elif sofa_hard_floor <= sofa_pct < sofa_soft_floor:
                    if any(kw in assessment_text for kw in soft_kw_sofa):
                        forced_solz = True

            # focal_anchor threshold: normal window-side uses 45/50; dining-middle note tightens it.
            # not_present (Q4 已自動 true) + percent=-1 也 force
            if isinstance(anchor_pct, (int, float)):
                if anchor_pct < 0:
                    # -1 = not_present
                    forced_focal = True
                elif anchor_pct < anchor_hard_floor:
                    forced_focal = True
                elif anchor_hard_floor <= anchor_pct < anchor_soft_floor:
                    if any(kw in anchor_text for kw in soft_kw_anchor):
                        forced_focal = True
            # focal_anchor_depth_position 顯示 not_present 也 force
            if result.get("focal_anchor_depth_position") == "not_present":
                forced_focal = True

            if forced_solz or forced_focal:
                # 把 flag 覆寫成 true + ok 同步覆寫成 false
                # （否則 z3_needs_retry 看到 ok=true 會 early-return）
                forced_reasons = []
                if forced_solz:
                    result["sofa_outside_living_zone"] = True
                    sp = int(sofa_pct) if isinstance(sofa_pct, (int, float)) else "?"
                    forced_reasons.append(
                        f"sofa 深度估 {sp}% (<{sofa_hard_floor} 硬 fail 或 "
                        f"{sofa_hard_floor}-{sofa_soft_floor} 含偏前描述)，視為未在靠窗 living zone"
                    )
                if forced_focal:
                    result["focal_anchor_misaligned_with_sofa"] = True
                    ap = int(anchor_pct) if isinstance(anchor_pct, (int, float)) else "?"
                    forced_reasons.append(
                        f"focal_anchor 深度估 {ap}% (<{anchor_hard_floor} 硬 fail 或 not_present)，與 sofa 未對齊"
                    )
                result["ok"] = False
                prev_reason = (result.get("reason") or "").strip()
