...
"""
    ...
    if has_layout_ctx:
        ...
        ws_kws = ["靠窗", "窗邊", "窗戶", "底端", "深處", "後半段", "後段",
                  "深端", "底部", "底側", "尾端", "末端",
                  "window", "back of the room", "back end", "deep end", "far end", "rear"]
        is_window_side = isinstance(living_w, str) and any(k in living_w for k in ws_kws)
        ...
        if is_window_side:
            ...
            # sofa threshold（user 規格：<60 硬 fail，60-65 看 reason）
            ...
            # focal_anchor threshold（user 規格：<45 硬 fail，45-50 看 assessment）
            ...
            if forced_solz or forced_focal:
                ...
                if forced_solz:
                    ...
                    forced_reasons.append(f"sofa 深度估 {sp}% (<60 硬 fail 或 60-65 含偏前描述)，視為未在靠窗 living zone")
                if forced_focal:
                    result["focal_anchor_misaligned_with_sofa"] = True
                    ap = int(anchor_pct) if isinstance(anchor_pct, (int, float)) else "?"
                    forced_reasons.append(f"focal_anchor 深度估 {ap}% (<45 硬 fail 或 not_present)，與 sofa 未對齊")
                result["ok"] = False
