...
def _sofa_focal_floor_depth_violation(rb: dict) -> tuple[float, float] | None:
    """用家具落地點檢查沙發與焦點櫃是否在同一條橫向觀看軸。"""
    if not isinstance(rb, dict):
        return None
...
"""
    ...
    if isinstance(rb, dict):
        ...
        # 沙發與 TV／焦點櫃必須在同一深度橫軸；不能只看左右對向就放行。
        if (room_type or "living") == "living":
            _depth_mis = _sofa_focal_floor_depth_violation(rb)
            if _depth_mis:
                _delta, _limit = _depth_mis
                result["focal_anchor_misaligned_with_sofa"] = True
                result["ok"] = False
                _tag = (f"[幾何] 沙發與焦點櫃落地深度差 {_delta:.0f} > {_limit:.0f}，"
                        "兩者未在同一觀看橫軸")
