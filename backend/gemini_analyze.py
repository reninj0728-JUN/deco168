...
def validate_render(
    ...
    if has_layout_ctx:
        ...
        layout_q_field = '  "sofa_outside_living_zone": bool,\n'
    else:
        layout_block = ""
        layout_q_field = ""
...
- sofa_faces_walkway = false 當沙發正面朝向：
  ...
  * 室內主視覺焦點（壁畫/景觀/裝飾牆）
""" + ("""
Q3: sofa_outside_living_zone（沙發是否明顯**違反**客戶已確認的 living zone）
- 上面已標示客戶確認的客廳區位置（binding）。判斷渲染圖內的沙發**主體**是否落在該區域：
  * 把原圖（image_1）跟確認文字一起看，比對渲染圖（image_2）內沙發的實際位置
  * 注意房間長軸方向：若確認區是「靠窗區域 / 深處 / 底端」，沙發應該明顯放在窗或房間底端那一側
  * 若確認區是「中段 / 前段 / 近門口」也同樣比對
- sofa_outside_living_zone = true 當以下成立（**only obvious 違反才回 true，避免過度誤判**）：
  * 沙發本體明顯放到 confirmed living zone **以外**的區域，例如：
    - 確認區是「深處靠窗」，但沙發出現在房間中段、入口側、或對面牆邊
    - 確認區是「前段近門口」，但沙發跑到房間底端
    - 沙發主要落在 dining zone、entrance zone、或主走道內
- sofa_outside_living_zone = false 當：
  * 沙發主體大致落在 confirmed living zone（小幅偏差、視角誤差不算）
  * 沒有 layout context 提供時（這題不適用）
- 寬鬆原則：**只在「明顯違反」才回 true**。如果 confirmed 描述為「深處靠窗區域」而沙發
  靠近窗但稍偏左右一點 → 回 false（不算明顯違反）。
- 若 sofa_outside_living_zone=true → reason 必須**明確寫**「沙發未在客戶確認的 living zone」
  或「沙發位置違反確認分區（應靠窗但放在中段）」這類具體說明。
