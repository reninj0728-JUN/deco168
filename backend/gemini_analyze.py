...
"""
    ...
    if has_layout_ctx:
        ...
        if is_window_side:
            ...
            # 門檻回合理值 (2026-06-23)：80% 太死（會把 75% 這種肉眼已靠窗的圖也擋掉→全滅）。
            # 改成「<~72% 硬傷、72–80% 軟交、80% 為理想目標」。質性條件（背窗/沒貼側牆/
            # TV未對齊/中段被佔）仍是硬傷，由各自 flag 處理，不靠這個深度數字。
            # 注意 strict 容差=5：hard_floor=77 → 實際 <72 才硬；anchor hard_floor=70 → <65 才硬。
            sofa_hard_floor = 77 if has_dining_middle_constraint else 60
            sofa_soft_floor = 80 if has_dining_middle_constraint else 65
            anchor_hard_floor = 70 if has_dining_middle_constraint else 45
            anchor_soft_floor = 80 if has_dining_middle_constraint else 50
