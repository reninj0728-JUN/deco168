...
【空間量測步驟 — 必須先做】
1. 找出可見的基準物：門框（高200cm/寬90cm）← 最可靠、窗台（距地90cm）、
   插座（距地30cm）、開關面板（距地120-135cm）、踢腳板（高6-10cm）
   沙發高80-90cm ← **只有畫面裡真的有家具時才用**；空屋沒有就跳過
2. 地板接縫：接縫看得見時，先用門框寬度（90cm）校準出「一格多大」再數格數。
   不要假設磚的規格（60×60／80×80／木紋磚20×120 都常見，畫面分不出來，
   猜錯就是 30% 以上誤差）；校準不出來就不要用這個方法
3. 用基準物推算房間的長度、寬度、天花板高度（公尺）
4. 交叉比對影片不同段落和各張照片的基準物
5. 用長×寬計算坪數（1坪=3.305㎡），給保守估計
6. room_dimensions.confidence 誠實標 high/medium/low：沒有可用基準物就填 low，
   系統會自動改用保守的家具尺寸上限，填 low 不會被扣分。
   **寧可誠實填 low，也不要硬給一個好看的數字。**
...
# 使用者裁決校準（living_layout_calibration.json）：
# 接受組最小 focal/door gap 約 0.290，拒絕組最大約 0.260。
# 0.28 位於兩群之間；任何調整必須先通過完整校準集。
...
def _living_bbox_is_broad_depth_zone(bbox) -> bool:
    """zone bbox 橫跨中後到前景時，不可用文字關鍵字硬當成「靠窗深端」。"""
    if not isinstance(bbox, (list, tuple)) or len(bbox) != 4:
        return False
    try:
        y0, _x0, y1, _x1 = [float(v) for v in bbox]
    except (TypeError, ValueError):
        return False
    if y1 < y0:
        y0, y1 = y1, y0
    return (y1 - y0) >= 450 and y1 >= 850
...
def _enforce_window_facing_field(result: dict, is_living: bool) -> dict:
    """客廳漏答窗戶朝向時 fail closed；不能把缺欄位當安全。"""
    if not is_living:
        result.setdefault("sofa_facing_window", False)
        result.setdefault("sofa_facing_window_unverified", False)
        return result
    if "sofa_facing_window" not in result:
        result["sofa_facing_window"] = False
        result["sofa_facing_window_unverified"] = True
        result["ok"] = False
        tag = "沙發是否正對主窗未完成驗證"
    else:
        result["sofa_facing_window_unverified"] = False
        if not result.get("sofa_facing_window"):
            return result
        result["ok"] = False
        tag = "沙發正面對主窗／落地窗"
    prev = (result.get("reason") or "").strip()
    result["reason"] = (prev + " | " + tag) if prev and "皆合理" not in prev else tag
    return result
...
def _enforce_sofa_focal_orientation(
    ...
    if is_long_room_layout:
        # 客戶指定左／右時才要求貼長牆；AI auto 可採前側小沙發或大客廳浮置。
        if not auto_layout and result.get("sofa_against_side_wall") is not True:
            result["sofa_outside_living_zone"] = True
...
def validate_render(
    ...
    auto_layout = bool(layout_context.get("auto_layout")) if has_layout_ctx else False
    if has_layout_ctx:
        ...
        main_window_wall = str(layout_context.get("main_window_wall") or "")
        auto_focal_side = str(layout_context.get("auto_focal_side") or "")
        auto_can_float = bool(layout_context.get("auto_can_float"))
        ...
        layout_block = (
            ...
            + (f"- 房間形狀：「{room_shape}」\n" if room_shape else "")
            + (f"- 主窗／採光開口：「{main_window_wall}」\n" if main_window_wall else "")
            + (f"- AI 自動配置：焦點牆={auto_focal_side or '由圖判斷'}，"
               f"可浮置={'是' if auto_can_float else '否'}\n" if auto_layout else "")
...
"""
    ...
    if is_long_room_layout:
        _wall_rule = (
            "- 本案為 AI 自動配置，不要求 sofa_against_side_wall=true；沙發可採前側小型或浮置，"
            "但 sofa_intrudes_walkway 與 coffee_table_in_walkway 都必須為 false。\n"
            if auto_layout else
            "- sofa_against_side_wall = true 只在沙發椅背完整貼齊並平行於左／右長牆時成立；"
            "指定靠左／靠右的訂單若不是 true 即不合格。\n"
        )
        long_room_validation_block = f"""
Q2b: 長條客廳與縱向走道（本案為長條／狹長格局，必須驗收）
{_wall_rule}- sofa_intrudes_walkway = true：沙發主體、扶手或 chaise 侵入入口通往房門／窗側的連續路徑，
  導致入口到後方開口不再有約 80–90 cm 寬的連續可走路徑。
- coffee_table_in_walkway = true：茶几或地毯讓入口到後方開口**沒有任何**約 80–90 cm
  寬的連續可走路徑，或人必須跨過家具才可通行。
- 正常客廳繞過茶几／地毯邊緣仍可順暢通行，必須填 false；不要把未經透視校正的
  粗略矩形框，當成整塊永久禁區。
"""
...
"""
    ...
    entrance_q_field = (
        '  "sofa_facing_entrance_door": bool,\n'
        '  "focal_anchor_past_door_in_depth": bool,\n'
        if _is_living_q else ""
    )
    window_q_field = ('  "sofa_facing_window": bool,\n' if _is_living_q else "")
    entrance_face_block = ("""
...
Q2f2: focal_anchor_past_door_in_depth（同牆透視防誤殺 — 客廳必答）
- 當大門在同一側牆的近景、TV／焦點櫃沿該牆位於更深處，且地磚、踢腳線、
  牆面消失線清楚證明兩者是前後錯開，不是同一深度並排 → true。
- 若櫃體真的貼著門框、侵入門扇掃掠／進門落腳區，或無法確認前後分離 → false。
- 不可只看 2D 水平 x-gap；近景大門會因透視放大，投影上看似貼近深處櫃體。
- 此欄只供診斷，不會自行豁免程式端的使用者校準門距。
""" if _is_living_q else "")
    window_face_block = ("""
Q2g: sofa_facing_window（沙發正面是否直接朝主窗／落地窗 — 客廳必答）
- 人坐在主沙發上正面直視，若主窗、落地窗或陽台玻璃門落在前方約 ±35 度 → true。
- 沙發正面朝電視／實牆，窗在側面、側後方或背後 → false。
- 小型側窗不算；只抓成為沙發主要視線終點的主窗／大面採光開口。
- true 是硬傷，ok 必須為 false；不能用「採光好」當理由放行。
""" if _is_living_q else "")

    product_q_field = (
        '  "product_sofa_seating_match": bool,\n'
        if exp_seat in ("single", "multi") else ""
    ) + product_visibility_field + entrance_q_field + window_q_field
    prompt = f"""
你會看到 2 張圖：
  image_1 = 原始空間照片
...
- 標完後回頭核對 Q1b：若 focal_anchor 或 sofa 真正侵入 entrance_door 的門扇掃掠、
  進門落腳區，或在同一深度貼門 → furniture_blocks_door 必須 true。
- 若只是近景門框與同牆深處 TV 櫃在 2D 投影上 x-gap 小，但透視證明前後分離，
  則 furniture_blocks_door=false 且 focal_anchor_past_door_in_depth=true。

【S2 GUIDE 污染判定 — HARD FAIL】
- guide_overlay_present = true 當 image_2 出現非室內設計元素的紅、綠、藍、黃色
  半透明區塊、框線、角點、標籤或連接線，尤其是沿沙發／TV／入口／走道輪廓的標示。
- 正常家具、牆面或軟裝本身的顏色不算；只抓明顯的配置輔助圖層與幾何標記。
- true 時成品不可交付，ok 必須為 false。
...
- furniture_blocks_walkway = true 當任一成立：
  * 沙發、茶几、地毯或大型家具明顯封住「左/右側走廊開口」或「主動線」，
    入口到後方不再有任何約 80–90 cm 寬的連續路徑，或人必須跨越家具才能通過
  * 沙發「浮」在房間中間（不靠牆），且本體位於走道路徑上
  * 茶几或地毯的一部分延伸到走道區，導致通道寬度看起來 < 60cm
  * 從入口走到窗邊或從入口走到房門開口，路徑被家具吃掉
  * L 型沙發的轉角或側邊伸入走廊開口前的淨空區
- furniture_blocks_walkway = false 當仍有約 80–90 cm 寬的連續可走路徑；路徑沿茶几或
  地毯邊緣自然轉彎是正常客廳動線，不算阻擋。
...
- furniture_blocks_door = false 當每扇門前方都有淨空、可直接走到，
  且大門旁一個門寬內沒有大型家具貼牆
- 透視例外必須與 Q2f2 一致：近景門框和同牆深處 TV 櫃在 2D 投影上看似相近，
  但地磚／踢腳線證明前後錯開、沒有侵入門扇掃掠或落腳區時，必須填 false。
...
- sofa_faces_walkway = true 當以下成立（**獨立於 Q1 判斷**）：
  ...
  * 沙發「背」靠的方向也對 — 看「正面朝向」哪個 vector：
    - 沙發正面朝 TV 牆/室內焦點 → false
    - 沙發正面朝 主窗/落地窗/走廊開口/房門/主走道/大門 → true
  * 注意：沙發**沒擋路**但「面對走道」依然 true（這是兩個獨立檢查）
- sofa_faces_walkway = false 當沙發正面朝向：
  * 對面 TV 牆（純實牆，沒開口）
  * 室內主視覺焦點（壁畫/裝飾牆）
{long_room_validation_block}{sofa_side_validation_block}{product_seat_block}{product_visibility_block}{entrance_face_block}{window_face_block}
""" + ("""
...
"""
    ...
    result.setdefault("focal_anchor_past_door_in_depth", False)
    result = _enforce_window_facing_field(result, is_living=_is_living_q)
    ...
    if isinstance(rb, dict):
        ...
        # ── BB034AB8 根治：門邊淨空改使用者校準幾何，不靠判官布林 ──
        ...
        if (room_type or "living") == "living":
            ...
            if _viol and not result.get("furniture_blocks_door"):
                _nm, _x_gap, _door_w, _thr = _viol
                result["furniture_blocks_door"] = True
                result["ok"] = False
                _tag = (f"[幾何] {_nm} 與大門間距 {_x_gap:.0f} < {_thr} 門寬"
                        f"（門寬 {_door_w:.0f}）——家具貼門/沙發視線掃到門")
    ...
    # E72F4ADB：底圖是「裁出來的客廳單房視角」時整個畫面都是客廳區——
    # 「在不在 zone」由裁切保證，深度百分比再罰是雙重審判；且版面引導圖
    # 已是擺位的單一真相源（判官同一張圖判「結構與動線皆合理」，卻被
    # C2.4 的深度門檻殺掉 = 新舊兩個主人打架）。裁切底圖 → C2.4 讓位。
    if has_layout_ctx and isinstance(layout_context, dict) \
            and layout_context.get("base_is_room_crop"):
        pass
    elif has_layout_ctx:
        ...
        broad_living_bbox = _living_bbox_is_broad_depth_zone(
            layout_context.get("living_bbox") if isinstance(layout_context, dict) else None)
    ...
    if result.get("guide_overlay_present"):
        result["ok"] = False
        previous_reason = (result.get("reason") or "").strip()
        overlay_reason = "成品殘留S2配置guide色線／色塊"
        if overlay_reason not in previous_reason:
            result["reason"] = (
                f"{overlay_reason}；{previous_reason}" if previous_reason else overlay_reason
            )
