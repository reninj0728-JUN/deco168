...
"""
    ...
    # 但 windows/walls_changed 全 false → 假及格交付）。三個「保留」預設 True、
    # 一個「入侵」預設 False（缺欄位＝不誤殺）；任一失守 → spatial_fidelity_fail。
    ...
    if not result.get("camera_axis_preserved", True):
        _fid_problems.append("相機視角被換成另一個空間")
    if not result.get("main_window_region_match", True):
        _fid_problems.append("主窗方位被移動")
    if not result.get("passage_openings_preserved", True):
        _fid_problems.append("原有走道門洞消失")
    if result.get("offframe_room_invaded", False):
        _fid_problems.append("畫面外的廚房/房間被畫進畫面")
    result["spatial_fidelity_fail"] = bool(_fid_problems)
    if _fid_problems:
        result["ok"] = False
