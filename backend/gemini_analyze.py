...
"""
    ...
    if has_layout_ctx:
        ...
        if is_window_side:
            ...
            # 硬傷 vs 軟傷分級 (2026-06-21)：
            #   - 純深度數字小偏差（如 55% vs 60%）→ 軟傷，照交付，不擋圖、不重生。
            #   - 數字 + Gemini 質性描述「沙發/焦點在前段/入口/餐廳等錯區」→ 硬傷（跑錯分區）。
            #   - 嚴重偏離（低於硬門檻 20 點以上）→ 硬傷，即使 Gemini 沒寫質性字也擋。
            #   - 焦點 not_present（根本沒有電視櫃/焦點家具）→ 硬傷（完全沒對向）。
            forced_solz = False        # 硬傷：沙發跑錯分區
            forced_focal = False       # 硬傷：焦點完全沒對向 / 嚴重錯位
            soft_notes = []            # 軟傷：可交付，只記錄
            sofa_gross_floor = sofa_hard_floor - 20
            anchor_gross_floor = anchor_hard_floor - 20

            if isinstance(sofa_pct, (int, float)) and sofa_pct < sofa_soft_floor:
                qual_wrong = any(kw in assessment_text for kw in soft_kw_sofa)
                if sofa_pct < sofa_gross_floor or (sofa_pct < sofa_hard_floor and qual_wrong):
                    forced_solz = True
                else:
                    soft_notes.append(
            ...
            if anchor_not_present:
                forced_focal = True
            elif isinstance(anchor_pct, (int, float)) and anchor_pct < anchor_soft_floor:
                qual_wrong = any(kw in anchor_text for kw in soft_kw_anchor)
                if anchor_pct < anchor_gross_floor or (anchor_pct < anchor_hard_floor and qual_wrong):
                    forced_focal = True
                else:
                    soft_notes.append(
