...
def _sofa_faces_door_wall(rb: dict) -> str | None:
    """6DA08412 根治：沙發面朝『大門那面長牆』的幾何判定（0-1000 bbox）。

    門在左/右長牆時，沙發若在『對面』那半 → 沙發正面朝向大門那面牆 = 體感
    『沙發對門』（即使電視櫃離門有 0.26 門寬、判官沒判 facing 也一樣）。
    正解是翻面：沙發跟大門同側（過門擺）、面朝無門牆。回門側字串 or None。

    只在「門明顯在側牆（左/右三分之一）」時判——門在畫面中央多是後牆/端景門，
    不適用此規則。沙發中心要明顯落在對側（留 100/1000 緩衝避免誤殺置中沙發）。"""
    if not isinstance(rb, dict):
        return None
    ...
    door = rb.get("entrance_door")
    sofa = rb.get("sofa")
    if not (_valid(door) and _valid(sofa)):
        return None
    door_cx = (door[1] + door[3]) / 2
    sofa_cx = (sofa[1] + sofa[3]) / 2
    if 300 < door_cx < 700:      # 門在中央 → 後牆/端景門，不套此規則
        return None
    door_side = "left" if door_cx < 500 else "right"
    # 沙發中心要明顯在對側（門在左→沙發中心 > 600；門在右→沙發中心 < 400）
    if door_side == "left" and sofa_cx > 600:
        return "left"
    if door_side == "right" and sofa_cx < 400:
        return "right"
    return None
...
"""
    ...
    if isinstance(rb, dict):
        ...
        if (room_type or "living") == "living":
            ...
            # 6DA08412：沙發面朝大門那面長牆（在對側）→ 結構性「沙發對門」。
            # 正解是翻面（沙發同門側過門擺）。走 furniture_blocks_door 進重試鏈，
            # 讓 DOOR-ON-A-LONG-WALL LAYOUT 預設把它翻正。
            _door_side = _sofa_faces_door_wall(rb)
            if _door_side and not result.get("furniture_blocks_door"):
                result["furniture_blocks_door"] = True
                result["ok"] = False
                _tag = (f"[幾何] 沙發在{'右' if _door_side=='left' else '左'}側、面朝"
                        f"大門所在的{'左' if _door_side=='left' else '右'}長牆——沙發對門，"
                        "應翻面（沙發改與大門同側、過門擺、面朝無門牆）")
                _prev = (result.get("reason") or "").strip()
                if _tag not in _prev:
                    result["reason"] = (_tag + " | " + _prev) if _prev and "皆合理" not in _prev else _tag
