...
"""
    ...
    if has_layout_ctx:
        ...
        if is_window_side:
            ...
            # 明確分區（客廳靠窗 + 餐廳中段）：沙發與電視櫃都必須 >=80% 才算靠窗，
            # 73–77% 這種「還是偏中段」直接硬傷、不軟交（no_soft）。focal 目標也拉到 80。
            no_soft = bool(has_dining_middle_constraint and strict_depth)
            anchor_target = 80 if no_soft else anchor_soft_floor
            ...
            if anchor_not_present:
                ...
                anchor_cls = _depth_classification(
                    anchor_pct, anchor_hard_floor, anchor_target, strict_depth,
                ...
                if anchor_cls == "hard":
                    ...
                    soft_notes.append(
                        f"focal_anchor 深度估 {int(anchor_pct)}% 略低於目標 {anchor_target}%（軟傷，照交付）"
                    )
