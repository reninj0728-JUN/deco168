...
Q2e: 產品可見性（購買清單的 must 商品，圖上有沒有「大致出現」）
本案客廳清單商品：
{_pv_lines}
對每一件判斷（key: {_pv_keys}）：
...
- 人坐在主沙發上正面直視的方向：大門門扇落在視線正前方約 ±35 度範圍內，
  或大門與電視/焦點櫃在同一面牆上「緊鄰」且沙發正面朝向該區 → true。
- 大門在沙發的側面、側後方、背後，或 image_2 根本看不到大門 → false。
- 這是體感判斷：客戶坐上沙發第一眼會不會「看著自家大門」？會 → true。
""" if _is_living_q else "")
...
"""
    ...
    if _must_products and isinstance(result.get("product_visibility"), dict):
        _pv_bad = [f"{k}:{v}" for k, v in result["product_visibility"].items()
                   if str(v).strip().lower() in ("missing", "different")]
        if _pv_bad:
            result["product_visibility_fail"] = True
            result["ok"] = False
            tag = "[產品可見性] 清單商品未如實出現：" + "、".join(_pv_bad[:4])
            prev = (result.get("reason") or "").strip()
            if tag not in prev:
                result["reason"] = (tag + " | " + prev) if prev and "皆合理" not in prev else tag
