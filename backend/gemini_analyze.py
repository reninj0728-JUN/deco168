...
# 門邊淨空及格線：櫃子（＝沙發視線落點）要求較嚴——6DA08412 客訴圖 0.26 門寬
# （坐沙發看電視時大門就在視線裡），客戶眼球認可圖 0.42。0.35 卡在中間：
# 客訴等級必死、認可等級穩過。沙發自身只要不進開門範圍即可（0.25）。
DOOR_GAP_MIN_FOCAL = 0.35
...
def _perspective_door_exception_proven(rb: dict) -> bool:
    ...
    sofa_center_y = (float(sofa[0]) + float(sofa[2])) / 2.0
    focal_center_y = (float(focal[0]) + float(focal[2])) / 2.0
    sofa_focal_axis_gap = abs(sofa_center_y - focal_center_y)
    return (
        door_to_focal >= 120.0
        and door_to_sofa >= 100.0
        and sofa_focal_axis_gap <= 40.0
    )
...
def _door_adjacency_violation(rb: dict,
    ...
    側牆透視壓縮會讓深處櫃子在影像上顯得離門近，門檻用實圖校準
    （客訴 0.26 / 認可 0.42），不誤殺、真貼門一定抓。"""
    if not isinstance(rb, dict):
        return None
...
def _sofa_focal_center_axis_violation(rb: dict) -> tuple[float, float] | None:
    """以 bbox 中心觀看軸檢查沙發與焦點櫃；落地線僅供門區深度判定。"""
    if not isinstance(rb, dict):
        return None
    sofa, focal = rb.get("sofa"), rb.get("focal_anchor")
    for b in (sofa, focal):
        if not (isinstance(b, (list, tuple)) and len(b) == 4
                and all(isinstance(v, (int, float)) for v in b)
                and b[2] > b[0] and b[3] > b[1]):
    ...
    sofa_center_y = (float(sofa[0]) + float(sofa[2])) / 2.0
    focal_center_y = (float(focal[0]) + float(focal[2])) / 2.0
    delta = abs(sofa_center_y - focal_center_y)
    # normalized 0-1000；中心觀看軸超過 4% 畫高，已是斜對而非正對。
    threshold = 40.0
...
- coffee_table_in_walkway = true：茶几或地毯位於房間縱向通道中央，只要行走需繞過就填 true。
- 入口到各開口必須保留約 80-90 cm 連續通道；任一家具擋路，ok 必須為 false。
"""
...
"""
    ...
    if isinstance(rb, dict):
        ...
        # 沙發與 TV／焦點櫃中心必須在同一觀看橫軸；落地線只判門區深度。
        if (room_type or "living") == "living":
            _axis_mis = _sofa_focal_center_axis_violation(rb)
            if _axis_mis:
                _delta, _limit = _axis_mis
                result["focal_anchor_misaligned_with_sofa"] = True
                result["ok"] = False
                _tag = (f"[幾何] 沙發與焦點櫃中心觀看軸差 {_delta:.0f} > {_limit:.0f}，"
                        "兩者是斜對而非正對")
        ...
        if (room_type or "living") == "living":
            _viol = _door_adjacency_violation(
                rb,
                focal_past_door_in_depth=bool(
                    result.get("focal_anchor_past_door_in_depth")),
            )
