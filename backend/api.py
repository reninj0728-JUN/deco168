...
def _parse_sofa_wall_cm(raw: str) -> int | None:
    """客人填的沙發牆寬（公分）。空白、不是數字、超出合理範圍都當沒填。"""
    from furniture_match import SOFA_WALL_RANGE_CM
    try:
...
def _crop_region_base(base_path: str, room_type: str, job_dir, idx: int) -> tuple[str, bool, str, bool]:
    ...
    try:
        ...
        # 比例鎖定 v2（用戶抓漏：1.29 的底圖被輸出成 1.5，多出來的畫面不是原照片
        # 的內容）。改「只裁不補」：在現有框內收斂成精確 3:2——太寬置中裁寬
        # （框已在門界內，安全）、太高偏下裁高（多裁天花板、保留家具/地板），
        # 底圖與輸出同比例 ⇒ 模型不必為了填滿輸出框而重構畫面。
        # ⚠️ 比例差只證明「一定會發生裁切／重構／補畫」，**不能證明它補在哪一側**；
        # 當時判斷「補畫區恰好蓋到門」是推論不是量測。這道鎖消除的是重構壓力本身。
        _TARGET_AR = 1.5
        cw, ch = (x1 - x0), (y1 - y0)
        ar = cw / max(1, ch)
    ...
    try:
        ...
        for _rk in distinct_rks:
            ...
            _lst = enrich_renders(renders_in, analysis=analysis,
                                  ...
                                  sofa_wall_cm=(sofa_wall_cm if _rt == "living" else None))
