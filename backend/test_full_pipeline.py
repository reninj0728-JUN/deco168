...
【空間量測步驟 — 必須先做】
1. 找出畫面中可見的基準物，優先用固定不動、規格標準的：
   - 門框：高 200cm、寬 90cm ← 最可靠，幾乎每張室內照都有
   - 插座面板距地 30cm；開關面板距地 120–135cm
   - 窗台距地 90cm
   - 踢腳板高 6–10cm
   - 沙發高 80–90cm、深 90cm ← **只有畫面裡真的有家具時才用**。
     本產品多數是空屋照，沒有家具就跳過這一項，不要假設有一張沙發在那裡。
2. 地板接縫數格子（空屋最有效，但**必須先校準**）：
   - 接縫看得見時，先拿門框寬度（90cm）比對出**一格的實際尺寸**，
     再沿牆數格數 × 該尺寸得到長度
   - 不要假設磚的規格：60×60、80×80、木紋磚 20×120 都很常見，
     照片裡分不出來，猜錯就是 30% 以上的誤差。校準不出來就不要用這個方法。
3. 交叉比對所有照片中的基準物，推算房間長度、寬度、天花板高度（公尺）
4. 找出各張照片拍攝方向，確認哪些牆面/角落已被覆蓋
5. 用長×寬計算坪數（1坪=3.305㎡），給保守估計
6. room_dimensions.confidence 要誠實填：
   - high   = 有兩個以上互相吻合的基準物，或地磚已校準且數得清
   - medium = 只有單一基準物，或角度太斜不好換算
   - low    = 沒有可用基準物，只能憑空間感覺猜
   填 low 不會被扣分：系統會自動改用保守的家具尺寸上限。
   **寧可誠實填 low，也不要硬給一個好看的數字。**
7. 特別記錄：天花板結構（明管/梁柱/灑水頭）、門的位置、窗戶位置——這些在渲染時必須保留
...
"""
    ...
    if video_path:
        try:
            ...
            # D50FC472/9FD21D7E 根治：Gemini 偶爾卡死在 PROCESSING 不動——舊迴圈
            # 無上限等待，整個 analyze 吊死 30 分鐘直到容器重啟、整單陣亡。
            # 影片永遠不是必要條件：等超過 120s 直接退回純照片模式，不卡單。
            _vid_t0 = time.time()
            while uploaded_video.state.name == "PROCESSING":
                if time.time() - _vid_t0 > 120:
                    print("  [影片] Gemini 卡在 PROCESSING 超過 120s——退回純靜態圖模式，不卡單")
                    uploaded_video = None
                    break
                print(f"  [影片] 處理中… state={uploaded_video.state.name}")
                time.sleep(3)
                uploaded_video = client.files.get(name=uploaded_video.name)
            if uploaded_video is not None and uploaded_video.state.name == "FAILED":
...
def _gpt_image2_mask_repair_prompt(reference_map: list[dict],
    ...
    if mask_mode == "console_door":
        ...
        return (
            ...
            "areas exactly. Output a clean photorealistic image with no red, blue, mask or guide."
        )
    return (
        ...
        "body target. Inside the WHITE editable mask area, completely erase the old sofa from the RED "
        ...
        "occupy or touch the RED old-sofa zone or the entrance landing. Preserve every pixel in "
        "BLACK mask areas: double entrance door, intercom, TV, console, coffee table, "
