...
# ── 非家具攔截（客訴：「簡約風刀具砧板架」被當「收納櫃」推薦給客戶）──
# 爬蟲會把賣場的廚具/耗材/家電一起帶進來，分類腳本再把它們塞進「收納」「裝飾」。
# 這裡是最後一道閘門：載入時就擋掉，任何下游（配對／軟裝／清單）都碰不到。
# 原則＝高精準度優先：只列「不管上下文都不可能是房間裡一件家具」的詞。
# 會誤傷合法家具的詞（架、櫃、桌、几…）一律不進這張表。
NON_FURNITURE_KW = [
    # 廚具餐具
    "砧板", "刀具", "菜刀", "鍋鏟", "平底鍋", "湯鍋", "不沾鍋", "鍋具", "鍋蓋",
    "瀝水", "餐盤", "碗盤", "馬克杯", "水杯", "玻璃杯", "筷", "湯匙", "叉子",
    "保鮮盒", "便當盒", "水壺", "冷水壺", "果汁壺", "茶壺", "咖啡壺", "烤盤",
    "開瓶器", "量杯", "調味罐", "食器", "餐墊", "隔熱墊", "防油濺",
    # 清潔耗材
    "清潔布", "抹布", "拖把", "掃把", "垃圾袋", "洗衣精", "洗碗精", "除臭包",
    "除濕劑", "防蟲", "樟腦", "乾燥劑", "竹炭包", "濕紙巾", "除塵紙", "菜瓜布",
    "垃圾桶", "紙簍",
    # 衛浴個人
    "牙刷", "馬桶刷", "肥皂", "沐浴乳", "洗手乳", "漱口", "化妝棉", "面紙盒",
    # 曬衣雜物（落地衣帽架是家具，塑膠衣架/曬衣架不是）
    "塑膠衣架", "曬衣架", "晾衣架", "褲架", "衣夾", "鞋撐", "領帶架",
    # 五金
    # 「螺絲」不可入表——「免螺絲角鋼電視櫃」是合法家具，只列工具本體
    "無痕掛鉤", "掛鉤組", "掛勾組", "螺絲起子", "電鑽", "工具箱", "膠帶",
    # 家電本體
    "吹風機", "電風扇", "空氣清淨", "電鍋", "微波爐", "烤箱", "冰箱", "洗衣機",
    "吸塵器", "電暖", "加濕器", "咖啡機",
    # 寵物、園藝工具
    "貓砂", "貓抓板", "寵物床", "狗窩", "貓窩", "飼料",
    "澆花器", "澆水壺", "灑水壺", "噴霧瓶", "噴水瓶",
    # 食品飲料（嚴格詞，不可用「茶」——會誤殺茶几）
    "菊花茶", "海鹽", "咖啡豆", "餅乾", "沖泡", "果汁粉", "奶粉", "零食包",
    "罐頭", "享樂券", "蜂蜜", "堅果",
]


def is_non_furniture(name_zh: str) -> bool:
    """這件商品是不是「根本不該出現在室內設計提案」的東西。"""
    nm = (name_zh or "")
    return any(k in nm for k in NON_FURNITURE_KW)
...
def load_catalog() -> list[dict]:
    ...
    if CATALOG_REAL_PATH.exists():
        with open(CATALOG_REAL_PATH, "r", encoding="utf-8") as f:
            catalog = json.load(f)
        _before = len(catalog)
        catalog = [x for x in catalog if not is_non_furniture(x.get("name_zh"))]
        if _before != len(catalog):
            print(f"[furniture_match] 非家具攔截: {_before - len(catalog)} 件不進推薦")
...
def violates_slot_guard(target_cat: str, room_mode: str, name_zh: str) -> bool:
    """品名宣告的主類型跟槽位衝突（例：槽位=茶几、品名=電視櫃）→ True。"""
    guards = SLOT_NAME_GUARDS.get(target_cat)
    if not guards:
        return False
    nm = (name_zh or "").lower()
    ...
    def _prefer_non_bundle(pool: list[dict]) -> list[dict]:
        ...
        for it in pool:
            ...
            if target_cat == "sofa" and infer_sofa_seating(nm, it.get("flux_descriptor") or "") == "single":
                continue
            # 槽位鐵則（50873CF0）：品名宣告的主類型跟槽位衝突就排除
            # （電視櫃不進茶几槽、鞋櫃不當電視櫃、書桌不當床頭桌、L型電視櫃不進主臥收納）
            if violates_slot_guard(target_cat, room_mode, nm):
                continue
            out.append(it)
        return out
...
def match_furniture(
    ...
    if remaining > 0:
        nice_pool = [
            ...
            # 槽位鐵則也管 nice 池（50873CF0：摺疊電腦桌被當床頭 side_table 塞進主臥）
            and not violates_slot_guard(resolve_category(it), mode, it.get("name_zh") or "")
        ]
...
# 寬度關鍵品類：這些品類的商品名裡出現的單一長度數字，幾乎都是「寬/總長」
# （電商慣例：沙發/櫃體/桌子標最長邊）。其他品類（窗簾/燈/床）名裡的數字
# 常是高度/直徑，不當寬度。
...
def extract_item_width_cm(item: dict) -> int | None:
    ...
    if w is not None:
        return w
    if cat in _WIDTH_CRITICAL_CATS:
        # 品名 fallback：寬NNN / N尺 / 裸NNNcm（帶非寬度前綴防呆）
        return _extract_width_cm(item.get("name_zh", ""), allow_bare=True)
    return None


def filter_by_dimensions(items: list[dict], max_width_cm: int) -> list[dict]:
    """過濾掉寬度超過 max_width_cm 的家具（dimensions 欄 + 品名 fallback）。"""
    if max_width_cm >= 280:  # 大空間不過濾
        return items
    result = []
    for item in items:
        w = extract_item_width_cm(item)
