...
- 🔴 **上線中的 9 種風格（以 `style-form.html` 為準，客戶只選得到這些）**：
  `modern` / `cream` / `nordic` / `japanese` / `wood` / `luxury` / `french` / `muji` / `chinese-modern`
  · **`art-deco` / `boho` / `industrial` / `mediterranean` 已停售**——客戶選不到這四種。
    ⚠️ 但「掛停售標籤」不等於「配不到」：**只掛停售標籤、沒有任何上線風格的商品才配不到**。
    目錄現有 48 件帶 `industrial`，第一標籤全是 modern／wood／cream／nordic，**客戶選那些風格照樣會配到**。
